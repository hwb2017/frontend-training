...
for (let i = 0; i < articleCount; i++) {
  articleList.push({
    ...
    abstractContent: faker.lorem.sentence(2),
    ...
    pageViews: faker.datatype.number({ min: 100, max: 500})
  })
}
...
export default class Article {
  ...
  async getArticles(ctx: any) {
    ...
    const pageList = mockList.filter((_, index) => index < (limit as number) * (page as number - 1));
    return {
