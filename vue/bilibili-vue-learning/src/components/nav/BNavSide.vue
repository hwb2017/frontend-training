	...
	<div class="nav-side" :class="{customizing: isSort}" ref="navSide">
		...
		<div class="nav-list" ref="list">
			<template v-for="(item, index) in data">
				<div :key="index" v-if="isDrag && index === replaceItem && replaceItem <= dragId" class="n-i sotrable">
					<div class="name"></div>
				</div>
                <!-- 在内联JavaScript语句调用方法时，可以通过$event访问原生的event对象 -->
				<div :key="index" class="n-i sotrable" :class="[{'on': current===index && !isSort}, {'drag': isDrag && current === index}]"  @click="setEnable(index)" @mousedown="dragStart($event, index)" :style="dragStyles">
...
export default {
  ...
  computed: {
    ...
    // 拖拽的元素的position会变为absolute, dragStyles 用来设置其位置，鼠标运动时会调用，从而实现跟随鼠标运动
    dragStyles() {
      return {
        left: `${this.x}px`,
        top: `${this.y}px`
    ...
    //当被拖拽的元素运动到其他元素的位置时，会使得replaceItem发生变化
    replaceItem() {
