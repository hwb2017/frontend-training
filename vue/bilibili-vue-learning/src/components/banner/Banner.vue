		...
		<div class="topic-preview-wrapper">
			<div class="topic-preview-list-wrapper">
				<ul class="topic-preview" style="width: 500%;" ref="banner"> 
					<BannerItem v-for="item in bannerlist" :banner="item" :key="item.id"></BannerItem>
				</ul>
			</div>
			...
			<div class="s-bottom">
				...
				<ul class="slide-bar">
					<li :class="{on: count == index}" v-for="(item, index) in bannerlist" @click="cutItem(index)" :key="item.id"></li>
				</ul>
			</div>
		</div>
	</div>
